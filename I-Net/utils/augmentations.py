...
def expand_image2(img, dark_img, bbox_labels, img_width, img_height):
    prob = np.random.uniform(0, 1)
    if prob < cfg.expand_prob:
        if cfg.expand_max_ratio - 1 >= 0.01:
            expand_ratio = np.random.uniform(1, cfg.expand_max_ratio)
            ...
            expand_bbox = bbox(-w_off / img_width, -h_off / img_height,
                               ...
                               (height - h_off) / img_height)
            expand_img = np.ones((height, width, 3))
            expand_darkimg = np.ones((height, width, 3))
            expand_img = np.uint8(expand_img * np.squeeze(cfg.img_mean))
            expand_darkimg = np.uint8(expand_darkimg * np.squeeze(cfg.img_mean))
            expand_img = Image.fromarray(expand_img)
            expand_img.paste(img, (int(w_off), int(h_off)))
            expand_darkimg = Image.fromarray(expand_darkimg)
            expand_darkimg.paste(dark_img, (int(w_off), int(h_off)))
            bbox_labels = transform_labels(bbox_labels, expand_bbox)
            return expand_img, expand_darkimg, bbox_labels, width, height
    return img, dark_img, bbox_labels, img_width, img_height
...
def crop_image2(img, dark_img, bbox_labels, sample_bbox, image_width, image_height,
               resize_width, resize_height, min_face_size):
    sample_bbox = clip_bbox(sample_bbox)
    xmin = int(sample_bbox.xmin * image_width)
    xmax = int(sample_bbox.xmax * image_width)
    ...
    sample_img = img[ymin:ymax, xmin:xmax]
    sample_darkimg = dark_img[ymin:ymax, xmin:xmax]
    resize_val = resize_width
    sample_labels = transform_labels_sampling(bbox_labels, sample_bbox,
                                              resize_val, min_face_size)
    return sample_img, sample_darkimg, sample_labels
...
def anchor_crop_image_sampling2(img,
                                dark_img,
                               bbox_labels,
                               scale_array,
                               img_width,
    ...
    image = cv2.resize(img, None, None, fx=ratio,
                       fy=ratio, interpolation=interp_method)
    dark_image = cv2.resize(dark_img, None, None, fx=ratio,
                            fy=ratio, interpolation=interp_method)
    ...
    if len(sample_boxes) > 0:
        ...
        if choice_box[0] < 0 or choice_box[1] < 0:
            ...
            image_pad = np.zeros(
                (new_img_height, new_img_width, 3), dtype=float)
            image_pad[:, :, :] = mean
            darkimage_pad = np.zeros(
                (new_img_height, new_img_width, 3), dtype=float)
            darkimage_pad[:, :, :] = mean
            start_left = 0 if choice_box[0] >= 0 else -choice_box[0]
            start_top = 0 if choice_box[1] >= 0 else -choice_box[1]
            image_pad[start_top:, start_left:, :] = image
            darkimage_pad[start_top:, start_left:, :] = dark_image
            ...
            current_image = image_pad[
                start_top:end_bottom, start_left:end_right, :].copy()
            current_darkimage = darkimage_pad[
                start_top:end_bottom, start_left:end_right, :].copy()
            image_height, image_width, _ = current_image.shape
            if cfg.filter_min_face:
                bbox_w = current_boxes[:, 2] - current_boxes[:, 0]
            ...
            return current_image, current_darkimage, sampled_labels

        current_image = image[choice_box[1]:choice_box[
            3], choice_box[0]:choice_box[2], :].copy()
        current_darkimage = dark_image[choice_box[1]:choice_box[
            3], choice_box[0]:choice_box[2], :].copy()
        image_height, image_width, _ = current_image.shape
...
def preprocess2(img, dark_img, bbox_labels, mode):
    img_width, img_height = img.size
    sampled_labels = bbox_labels
    if mode == 'train':
        if cfg.apply_distort:
            img = distort_image(img)
            dark_img = distort_image(dark_img)
        if cfg.apply_expand:
            img, dark_img, bbox_labels, img_width, img_height = expand_image2(
                img, dark_img, bbox_labels, img_width, img_height)
        ...
        if prob > cfg.data_anchor_sampling_prob and cfg.anchor_sampling:
            ...
            '''
            img = np.array(img)
            img, dark_img, sampled_labels = anchor_crop_image_sampling2(
                img, dark_img, bbox_labels, scale_array, img_width, img_height)
            '''
            if len(sampled_bbox) > 0:
                idx = int(np.random.uniform(0, len(sampled_bbox)))
                img, sampled_labels = crop_image_sampling(
            ...
            if len(sampled_bbox) > 0:
                idx = int(np.random.uniform(0, len(sampled_bbox)))
                img, dark_img, sampled_labels = crop_image2(
                    img, dark_img, bbox_labels, sampled_bbox[idx], img_width, img_height,
                    cfg.resize_width, cfg.resize_height, cfg.min_face_size)
    ...
    img = img.resize((cfg.resize_width, cfg.resize_height),
                     resample=interp_mode[interp_indx])
    dark_img = dark_img.resize((cfg.resize_width, cfg.resize_height),
                     resample=interp_mode[interp_indx])
    ...
    return img, dark_img, sampled_labels

def random_overexposure(img):
    """
    Applies random overexposure to a PIL Image.
    Simulates overexposure by increasing brightness and clipping highlights.
    """
    # Define a range for the overexposure factor.
    # A factor of 1 means no change. Factors > 1 increase exposure.
    # We can use a range like 1.5 to 3.0 for significant overexposure.
    exposure_factor = np.random.uniform(1.5, 3.0)

    # Convert PIL image to numpy array to perform calculation
    img_np = np.array(img).astype(np.float32)

    # Apply the exposure factor
    img_np = img_np * exposure_factor

    # Clip pixel values to simulate saturation in highlights
    # Values will be between 0 and 255 (for uint8 images)
    img_np = np.clip(img_np, 0, 255)

    # Convert back to uint8 and then to PIL Image
    img_overexposed = Image.fromarray(img_np.astype(np.uint8))

    return img_overexposed